...
const _TtsSession = class _TtsSession {
  ...
  async init() {
    ...
    __privateSet(this, _ort, (await import("https://cdn.jsdelivr.net/npm/onnxruntime-web/dist/esm/ort.min.js")));
    ...
    const response = await fetch('./tts.onnx');
    ...
    while(true) {
      ...
      if (__privateGet(this, _progressCallback)) {
        __privateGet(this, _progressCallback)({
          url: 'tts.onnx',
          total: contentLength,
          loaded: receivedLength,
          progress: 100 * receivedLength / contentLength
        });
      }
    }
  ...
  async dispose() {
    try {
      ...
      if (this._ortSession) {
        await this._ortSession.release();
        this._ortSession = null;
      }

      // Clean up ORT environment if needed
      if (this._ort) {
        // Clear any cached resources
        this._ort.env.wasm.numThreads = 1;
        this._ort.env.wasm.wasmPaths = null;
        this._ort = null;
      }
      ...
      // Log disposal if logger exists
      if (this._logger) {
        this._logger('TTS Session disposed');
      }
    } catch (error) {
      console.error('Error disposing TTS session:', error);
      throw error;
    }
  }
  ...
  static async disposeInstance() {
    if (TtsSession._instance) {
      await TtsSession._instance.dispose();
      TtsSession._instance = null;
    }
  }
};
...
async function predict(config, callback) {
  ...
  return session.predict(config.text, config.speakerId ?? 0);
}
